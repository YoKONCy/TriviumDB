...
> **BQ 元数据块** 从 v6 开始采用自描述、固定小端序布局，显式记录块版本、每签名 chunk 数和签名数量，不再把 Rust 内存结构直接当作永久磁盘 ABI。加载器可读取 TriviumDB 0.7.0 的 v5/32-chunk 布局，以及 0.7.1–0.8.x 的 v5/48-chunk 布局；Writer 下一次 `flush()` 或 `close()` 会原子升级为 v6。
>
