...
use serde_json::json;
use triviumdb::query::cascades::{OptimizerBudget, PhysicalOperator, optimize_pipeline};
use triviumdb::query::pipeline::{
    ExactRerank, NodeIdsSource, PayloadFilter, PipelineBudget, PipelineContext, PipelineOperator,
    execute_pipeline,
};
use triviumdb::query::tql_executor::execute_tql;
...
fn similarity_仅在下游引用时插入精确重排() {
    ...
    assert!(scored_plan.stages.iter().any(|stage| {
        stage.operator == PhysicalOperator::ExactRerank && stage.vector_page_reads > 0
    }));
    assert!(
        !plain_plan
            .stages
            .iter()
            .skip(1)
            .any(|stage| stage.operator == PhysicalOperator::ExactRerank)
    );
}
...
fn graphstats_标签扇出影响_expand_基数估算() {
    ...
    let labeled_rows = labeled
        .stages
        .iter()
        .find(|stage| stage.operator == PhysicalOperator::GraphExpand)
        .unwrap()
        .estimated_rows;
...
fn 相关字段对修正独立性假设导致的基数低估() {
    ...
    let filter = plan
        .stages
        .iter()
        .find(|stage| stage.operator == PhysicalOperator::PayloadFilter)
        .unwrap();
