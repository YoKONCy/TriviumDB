...
fn 全表分页按节点ID稳定且差集不会遗漏() {
    ...
    let first = db.tql("MATCH (n) RETURN n LIMIT 10").unwrap();
    let ids = first.iter().map(|row| row["n"].id).collect::<Vec<_>>();
    assert_eq!(ids, (1..=10).collect::<Vec<_>>());
    db.create_index("skip").unwrap();
    let kept = db
        .tql(r#"FIND {skip: {$ne: true}} RETURN *"#)
        .unwrap()
        .into_iter()
        .map(|row| row["_"].id)
        .collect::<Vec<_>>();
    assert_eq!(kept, (1..=30).filter(|id| id % 3 != 0).collect::<Vec<_>>());
    drop(db);
    cleanup(&path);
}
...
fn TQL_FIND和MATCH超过五千行仍完整分页() {
    ...
    assert_eq!(
        db.tql(r#"FIND {type: "page"} RETURN *"#).unwrap().len(),
        6_001
    );
    assert_eq!(
        db.tql(r#"FIND {type: "page"} RETURN * LIMIT 6001"#)
            .unwrap()
            .len(),
        6_001
    );
    assert_eq!(
        db.tql(r#"FIND {type: "page"} RETURN * LIMIT 1001 OFFSET 5000"#)
            .unwrap()
            .len(),
        1_001
    );
    assert_eq!(db.tql("MATCH (n) RETURN n").unwrap().len(), 6_001);
    assert_eq!(
        db.tql("MATCH (n) RETURN n LIMIT 1001 OFFSET 5000")
            .unwrap()
            .len(),
        1_001
    );
...
fn 显式LIMIT可覆盖配置行上限但OFFSET仍受内存硬上限() {
    ...
    let overflow = db.tql("MATCH (n) RETURN n").unwrap_err();
    assert!(overflow.to_string().contains("超过 2 行上限"));
    assert_eq!(db.tql("MATCH (n) RETURN n LIMIT 3").unwrap().len(), 3);
    let memory_error = db.tql("MATCH (n) RETURN n LIMIT 2 OFFSET 2").unwrap_err();
    assert!(
        memory_error
            .to_string()
...
fn Break仅允许普通查询返回部分结果() {
    ...
    assert_eq!(db.tql("MATCH (n) RETURN n").unwrap().len(), 2);
    let aggregate_error = db
        .tql_values("MATCH (n) RETURN count(*) AS total")
        .unwrap_err();
    assert!(aggregate_error.to_string().contains("超过 2 行上限"));
    let ordered_error = db.tql("MATCH (n) RETURN n ORDER BY n.kind").unwrap_err();
...
fn TQL显式分页超过内存硬预算必须失败而非静默截断() {
    ...
    .unwrap();
    let error = db.tql("MATCH (n) RETURN n LIMIT 10 OFFSET 1").unwrap_err();
