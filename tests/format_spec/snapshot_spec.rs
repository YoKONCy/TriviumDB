...
use super::spec::{FLUSH_MARKER_V2, FileRole, TDB_HEADER};
use triviumdb::database::Database;

fn marker_v1_from_v2(bytes: &[u8]) -> Vec<u8> {
...
#[test]
fn flush_marker_v1兼容_v2完整字段边界_marker_crc和未知版本矩阵() {
    let source = seed("flush_marker_source");
    let marker = std::fs::read(format!("{source}.flush_ok")).unwrap();
    assert_eq!(marker.len(), 41);
    assert_eq!(marker[4], 2);

    std::fs::write(format!("{source}.flush_ok"), marker_v1_from_v2(&marker)).unwrap();
    ...
    let mutations = field_boundary_mutations(FLUSH_MARKER_V2)
        .into_iter()
        .chain(truncation_mutations(marker.len(), FLUSH_MARKER_V2))
        .chain([
            Mutation::FlipBit { offset: 37, bit: 0 },
            Mutation::Append(vec![0xa5; 8]),
        ]);
    for (index, mutation) in mutations.enumerate() {
        let candidate = path(&format!("flush_marker_case_{index}"));
        copy_roles(&source, &candidate, &[FileRole::Tdb, FileRole::Vec]);
...
fn marker修复自身crc后_错误尺寸和文件crc仍被语义校验拒绝() {
    ...
    let marker = std::fs::read(format!("{source}.flush_ok")).unwrap();
    for field_name in ["tdb_size", "vec_size", "tdb_crc", "vec_crc"] {
        let field = *FLUSH_MARKER_V2
            .iter()
...
fn tdb头部每个字段边界与逐字段截断均不panic且只读零写() {
    ...
    for (index, mutation) in mutations.enumerate() {
        ...
        copy_roles(&source, &candidate, &[FileRole::Vec, FileRole::FlushMarker]);
...
fn vec长度元素边界和非有限值由跨文件校验拒绝() {
    ...
    for (index, variant) in variants.into_iter().enumerate() {
        ...
        copy_roles(&source, &candidate, &[FileRole::Tdb, FileRole::FlushMarker]);
