...
> **状态**: Phase 5 论文定稿 + arXiv 投稿完成（九数据集全覆盖 + hnswlib 对照 + 四级适用性梯度 + 不可能三角因果分析）

> [!IMPORTANT]
> **算法正式命名（2026-05-02）**：本系统实现的核心 ANN 算法正式命名为 **QuIVer**  
> （**Qu**antized **I**ndex for **V**ector R**e**t**r**ieval）  
> 论文标题：_QuIVer: Rethinking ANN Graph Topology via Training-Free Binary Quantization_  
> §1–§11 保留历史探索时的原始名称（BQ-Vamana / BQ-HNSW）作为技术研发记录；§12–§14 使用正式名称 QuIVer。
...
### Phase 5: 全面验证与论文定稿 (05-02 ~ 05-04)

14. [九数据集全覆盖验证与论文投稿](#14-九数据集全覆盖验证与论文投稿)
...
| MiniLM-1M (LLM, 384d)      | 1M      | ef=64 (88%R)    | 41106 MT-QPS             | 83s         | ✅ SOTA         |
| Wolt-CLIP-1M (CLIP, 512d)  | 1M      | 无法达到        | 62367 MT-QPS (71%R)      | 58s         | 🔶 多模态高     |
| Cohere-1M (LLM, 768d)      | 1M      | ef=64 (95%R)    | 36729 MT-QPS             | 95s         | ✅ SOTA         |
| BGE-M3-1M (LLM, 1024d)     | 1M      | ef=32 (85%R)    | 41182 MT-QPS (94%R@ef64) | 78s         | ✅ SOTA         |
| DBpedia-1M (LLM, 1536d)    | 990K    | ef=64 (95%R)    | 22035 MT-QPS             | 142s        | ✅ SOTA         |
| DBpedia-3072 (LLM, 3072d)  | 990K    | ef=64 (96%R)    | 12910 MT-QPS             | 262s        | ✅ SOTA         |
| Synthetic-LR (低秩, 768d)  | 1M      | 无法达到        | 28091 MT-QPS (42%R)      | 125s        | 🔶 因果验证     |
| Random-Sphere (随机, 768d) | 1M      | 无法达到        | 24314 MT-QPS (0.4%R)     | 153s        | ❌ 结构坍缩     |
| GloVe-100 (词向量, 100d)   | 1.18M   | 无法达到        | 59372 MT-QPS (32%R)      | 82s         | 🔶 低维边界     |
| SIFT-128 (CV 局部, 128d)   | 1M      | 无法达到        | 87212 MT-QPS (15%R)      | 56s         | ❌ 度量不匹配   |
| GIST-960 (CV 全局, 960d)   | 1M      | 无法达到        | 103832 MT-QPS (2%R)      | 75s         | ❌ 度量不匹配   |
| **hnswlib 对照 (768d)**    | **1M**  | **ef=64**       | **QuIVer 2.1×快**        | **284s**    | ✅ **基线对照** |
...
graph LR
    ...
    I --> J["📝 Phase 5<br/>九数据集 + hnswlib<br/>+ 论文定稿 + arXiv"]

    style I fill:#2d6,color:#fff,stroke:#fff
    style J fill:#36c,color:#fff,stroke:#fff
```

### 10.3 关键数字摘要（Phase 5 最终版 · 百万级 bench_sensitivity 数据）

| 指标                     | 384-d (MiniLM) | 768-d (Cohere) | 1024-d (BGE-M3) | 1536-d (DBpedia) | 3072-d (DBpedia) |
| ------------------------ | -------------- | -------------- | --------------- | ---------------- | ---------------- |
| 每向量 BQ 内存           | 128B (1/12)    | 256B (1/12)    | 340B (1/12)     | 512B (1/12)      | 1024B (1/12)     |
| 建图时间 (m=32,ef_c=128) | 83s            | 95s            | 78s             | 142s             | 262s             |
| ef=64 Recall@10          | 88.09%         | 95.13%         | 93.81%          | 95.34%           | 95.65%           |
| ef=64 MT-QPS (16T)       | 41,106         | 36,729         | 41,182          | 22,035           | 12,910           |
| ef=64 1T-QPS             | 4,377          | 3,555          | 4,930           | 3,111            | 2,352            |
| ef=128 Recall@10         | 93.82%         | 97.69%         | 97.06%          | 97.74%           | 97.94%           |
| ef=128 MT-QPS            | 23,506         | 21,021         | 25,911          | 12,996           | 7,428            |
| 16T/1T 加速比 (ef=64)    | 9.5×           | 10.4×          | 8.8×            | 6.8×             | 5.4×             |
| Hot 内存                 | 583 MB         | 675 MB         | 736 MB          | 849 MB           | 1,212 MB         |
...
6. **经典数据集对照划定适用边界**：GloVe-100 / SIFT-128 / GIST-960 验证了 QuIVer 的不可能三角
7. **Phase 5 九数据集全覆盖 + 论文定稿 + arXiv 投稿**：MiniLM/DBpedia/RedCaps/Synthetic-LR/Random-Sphere 补全实验矩阵，提炼四级适用性梯度（Collapse / Usable / High / SOTA），完成 hnswlib 16× 对照，论文定稿并提交 arXiv
...
| ~~P0~~ | ~~HNSWLib / USearch 同机对照~~                    | ✅ 已完成 (hnswlib 16×, USearch 5×) |
| ~~P1~~ | ~~标准数据集评测 (SIFT/GIST/GloVe)~~              | ✅ 已完成                           |
| ~~P1~~ | ~~扩展数据集 (MiniLM/DBpedia/RedCaps/Synthetic)~~ | ✅ 已完成                           |
| ~~P0~~ | ~~论文定稿 + arXiv 投稿~~                         | ✅ 已完成 (2026-05-04)              |
...
**搜索性能**（bench_sensitivity 最终数据，m=32, ef_c=128, α=1.2）：

| ef_search | Recall@10  | MT-QPS (16T) | 1T-QPS | lat(ms) | vs BF    |
| --------- | ---------- | ------------ | ------ | ------- | -------- |
| 64        | **95.13%** | **36,729**   | 3,555  | 0.28    | 1,991.5× |
| 128       | 97.69%     | 21,021       | 2,140  | 0.47    | 1,139.8× |
| 256       | 98.96%     | 11,837       | 1,182  | 0.85    | 641.8×   |
| 512       | 99.55%     | 6,032        | 644    | 1.55    | 327.1×   |
| 1024      | 99.71%     | 3,376        | 355    | 2.82    | 183.0×   |

Brute-force 基线：

| 基线               | QPS  |
| ------------------ | ---- |
| Brute-force (1K q) | 18.4 |

> [!IMPORTANT]
> **这组数据是当前最关键的 SOTA 结果：**
> 在 100 万条真实 768 维 LLM 向量上，`ef=64` 已经达到 **95.13% Recall@10**，
> 16 线程查询吞吐达到 **36,729 QPS**，单线程 3,555 QPS。
> 线程扩展比约 10.4×，相对 Brute-force 是 **1,991×** 加速。
...
**搜索性能**（bench_sensitivity 最终数据）：

| ef_search | Recall@10  | MT-QPS (16T) | 1T-QPS | lat(ms) | vs BF    |
| --------- | ---------- | ------------ | ------ | ------- | -------- |
| 64        | **95.34%** | **22,035**   | 3,111  | 0.32    | 2,735.8× |
| 128       | 97.74%     | 12,996       | 1,893  | 0.53    | 1,613.5× |
| 256       | 98.83%     | 7,309        | 1,077  | 0.93    | 907.5×   |
| 512       | 99.33%     | 3,969        | 543    | 1.84    | 492.8×   |
| 1024      | 99.61%     | 2,104        | 318    | 3.15    | 261.2×   |

Brute-force 基线：

| 基线               | QPS |
| ------------------ | --- |
| Brute-force (1K q) | 8.1 |

这个结果非常关键，因为它把 §13.3 的 Cohere-1M 百万级结果进一步推进到了
**1536 维真实文本 embedding**。对比可以更清晰地看到：

| 数据集       | 维度 | 构建时间 | Hot 内存 | ef=64 R@10 | ef=64 MT-QPS | 16T/1T比 |
| ------------ | ---- | -------- | -------- | ---------- | ------------ | -------- |
| MiniLM-1M    | 384  | 83s      | 583 MB   | 88.09%     | 41,106       | 9.5×     |
| Cohere-1M    | 768  | 95s      | 675 MB   | 95.13%     | 36,729       | 10.4×    |
| BGE-M3-1M    | 1024 | 78s      | 736 MB   | 93.81%     | 41,182       | 8.8×     |
| DBpedia-1M   | 1536 | 142s     | 849 MB   | 95.34%     | 22,035       | 6.8×     |
| DBpedia-3072 | 3072 | 262s     | 1,212 MB | 95.65%     | 12,910       | 5.4×     |
...
| ------------------ | --------- | --------- |
| 构建时间           | 153s      | 95s       |
| Hot 内存           | 675 MB    | 675 MB    |
| ef=64 MT-QPS (16T) | 24,314    | 36,729    |
| ef=64 1T-QPS       | 2,791     | 3,555     |
| Brute-force QPS    | 18.7      | 18.4      |
...
| ef_search | Recall@10  | MT-QPS (16T) | 1T-QPS | lat(ms) | vs BF  |
| --------- | ---------- | ------------ | ------ | ------- | ------ |
| 64        | **32.08%** | **59,372**   | 5,115  | 0.20    | 467.1× |
| 128       | 44.86%     | 32,873       | 2,794  | 0.36    | 258.6× |
| 256       | 55.24%     | 17,818       | 1,561  | 0.64    | 140.2× |
| 512       | 63.57%     | 9,438        | 844    | 1.19    | 74.3×  |
| 1024      | 70.67%     | 4,846        | 445    | 2.25    | 38.1×  |

| 指标                  | 数值   |
| --------------------- | ------ |
| 构建时间              | 81.9s  |
| Hot 内存              | 618 MB |
| Brute-force QPS       | 127.1  |
| 16T/1T 加速比 (ef=64) | 10.6×  |

#### SIFT-128（1,000,000 × 128 维，图像局部特征）

| ef_search | Recall@10  | MT-QPS (16T) | 1T-QPS | lat(ms) | vs BF  |
| --------- | ---------- | ------------ | ------ | ------- | ------ |
| 64        | **14.85%** | **87,212**   | 7,368  | 0.14    | 736.8× |
| 128       | 20.40%     | 49,679       | 4,226  | 0.24    | 419.7× |
| 256       | 27.18%     | 27,442       | 2,382  | 0.42    | 231.8× |
| 512       | 34.73%     | 14,788       | 1,333  | 0.75    | 124.9× |
| 1024      | 43.01%     | 7,835        | 734    | 1.36    | 66.2×  |

| 指标                  | 数值   |
| --------------------- | ------ |
| 构建时间              | 55.6s  |
| Hot 内存              | 522 MB |
| Brute-force QPS       | 118.4  |
| 16T/1T 加速比 (ef=64) | 10.6×  |
...
| ef_search | Recall@10 | MT-QPS (16T) | 1T-QPS | lat(ms) | vs BF    |
| --------- | --------- | ------------ | ------ | ------- | -------- |
| 64        | **2.01%** | **103,832**  | 11,657 | 0.09    | 6,956.2× |
| 128       | 2.70%     | 59,094       | 6,629  | 0.15    | 3,958.9× |
| 256       | 3.38%     | 31,644       | 3,783  | 0.26    | 2,120.0× |
| 512       | 3.90%     | 16,459       | 1,996  | 0.50    | 1,102.7× |
| 1024      | 4.22%     | 8,176        | 996    | 1.00    | 547.7×   |

| 指标                  | 数值   |
| --------------------- | ------ |
| 构建时间              | 75.2s  |
| Hot 内存              | 720 MB |
| Brute-force QPS       | 14.9   |
| 16T/1T 加速比 (ef=64) | 9.0×   |

#### 跨数据集对比分析

将三组经典数据集与已有的 Cohere-1M、DBpedia-1M、Random-1M 放在一起：

| 数据集        | 维度 | 分布类型                | ef=64 Recall | ef=1024 Recall | ef=64 MT-QPS |
| ------------- | ---- | ----------------------- | ------------ | -------------- | ------------ |
| Random-Sphere | 768  | 均匀随机                | 0.40%        | 6.55%          | 24,314       |
| GIST-960      | 960  | Euclidean CV 全局描述子 | 2.01%        | 4.22%          | 103,832      |
| SIFT-128      | 128  | Euclidean CV 局部特征   | 14.85%       | 43.01%         | 87,212       |
| GloVe-100     | 100  | Angular 词向量          | 32.08%       | 70.67%         | 59,372       |
| Wolt-CLIP     | 512  | CLIP 对比学习           | 70.68%       | 85.68%         | 62,367       |
| Synthetic-LR  | 768  | 低秩合成                | 41.76%       | 89.57%         | 28,091       |
| MiniLM-1M     | 384  | LLM 对比学习 embedding  | 88.09%       | 99.17%         | 41,106       |
| BGE-M3-1M     | 1024 | LLM 对比学习 embedding  | 93.81%       | 99.57%         | 41,182       |
| Cohere-1M     | 768  | LLM 对比学习 embedding  | 95.13%       | 99.71%         | 36,729       |
| DBpedia-1M    | 1536 | LLM 对比学习 embedding  | 95.34%       | 99.61%         | 22,035       |
| DBpedia-3072  | 3072 | LLM 对比学习 embedding  | 95.65%       | 99.61%         | 12,910       |
...
### 13.9.4 Recall@K 多维度测试：BQ2 排序精度的天然上限

> **日期**: 2026-05-05
> **目的**: 测试不同 K 值下的 recall 表现，精确定位 BQ2 对称距离排序精度的天花板。
> **基准**: Cohere-1M, GT 1000 query × top-1000 真值, m=32, ef_c=128, α=1.2。
> **代码**: `benches/bench_recall_at_k.rs`

| ef | R@1(%) | R@10(%) | R@100(%) | R@500(%) |
|---|---|---|---|---|
| 8 | 68.70 | 55.67 | 62.56 | 59.00 |
| 32 | 92.00 | 88.56 | 62.56 | 59.00 |
| 64 | 96.00 | 95.08 | 62.56 | 59.00 |
| 128 | 97.60 | 97.69 | 69.99 | 59.00 |
| 256 | 98.40 | 98.94 | 85.83 | 59.00 |
| 512 | 99.10 | 99.56 | 94.08 | 59.70 |
| 1024 | 99.20 | 99.71 | 97.92 | 78.82 |
| 1536 | 99.20 | 99.71 | 98.93 | 87.10 |
| 2048 | 99.40 | 99.76 | 99.28 | 91.38 |

这组数据揭示了 BQ2 对称距离排序精度的一个非常关键的结构性约束。

**关键发现一：R@1 比 R@10 更容易达标。**
ef=64 时 R@1=96.0% vs R@10=95.1%——只需找对 1 个最近邻时，BQ2 的局部排序噪声影响更小。
这与 §13.9 的"可达性 vs 导航效率"框架一致：真实最近邻几乎总是在可达路径上。

**关键发现二：R@100/R@500 在 ef < K 时被钳位在 ~59%。**
代码中 `actual_ef = ef.max(k)`，所以当 ef < K 时，beam search 实际使用 ef=K。
此时返回的 K 个候选中，只有约 59% 是 f32 真正的 top-K——
这个 **~59% 就是 BQ2 对称距离在 Cohere-768 上的暴力扫描排序精度天花板**。

**关键发现三：大 K 下 BQ 噪声是主要瓶颈。**
R@500 到 91% 需要 ef=2048（~4× over-retrieve），说明越深的排序结果越需要大幅超采来补偿 BQ 排序噪声。
这进一步印证了 BQ2 的正确用法：**用 BQ 做图导航（只需可达性），用 f32 rerank 修复最终排序**。

> [!IMPORTANT]
> **~59% 这个数字极其重要。**
> 它意味着：如果你暴力扫描 100 万条 BQ2 签名并取 top-K，
> 只有约 59% 的结果和 f32 cosine 的真正 top-K 重合。
> 但图搜索 + f32 rerank 能把 R@10 拉到 95%+，
> 这正是 BQ-native graph navigation 的价值——**图拓扑比暴力排序更有效**。

### 13.9.5 BQ2 vs RaBitQ 1-bit：为什么不用随机旋转

> **日期**: 2026-05-05
> **目的**: 对比 BQ2 (2-bit Sign-Magnitude, 无旋转) 与 RaBitQ (1-bit, 随机旋转) 在排序精度上的差异。
> **方法**: Cohere-1M 中抽 100K 训练向量 + 200 查询，暴力扫描 top-K 与 f32 cosine 真值对比重合率。
> **RaBitQ 实现**: 遵循官方 FhtKacRotator（4 轮 flip-FHT-rescale + Kac's walk，768 维无零填充）。
> **代码**: `benches/bench_rbq2_precision.rs`

| K | BQ2 (2-bit, 无旋转) | RaBitQ-sym (1-bit, 旋转+hamming) | RaBitQ-asym (1-bit, 旋转+f32 query+修正) |
|---|---|---|---|
| 1 | **66.50%** | 35.50% | 21.00% |
...
这组对照实验回答了一个审稿人很可能会问的问题：

> **为什么不用 RaBitQ 的随机旋转方案？旋转不是能让量化误差更均匀吗？**

答案非常清晰：**在 cosine-native 对比学习嵌入上，旋转有害无益。**

1. **BQ2 在所有 K 上全面碾压 RaBitQ 1-bit**（+50%~66% 绝对优势）。
   RaBitQ-sym（旋转+对称 hamming）只有 35-42%，远低于 BQ2 的 59-66%。

2. **RaBitQ-asym（带修正项）反而最差**——修正项为 L2 度量设计，
   在 cosine-native 嵌入上完全失效，精度甚至低于纯对称版本。

3. **根本原因：contrastive learning 嵌入的维度幅值分布不均匀，这是有价值的先验。**
   BQ2 的 magnitude bit 利用了这个不均匀性（高幅值维度权重更大），
   而 RaBitQ 的随机旋转将其均匀化，反而丢失了信息。

4. **验证了 WHT 旋转 + 2-bit 叠加方案（RBQ2）也不如纯 BQ2**：
   旋转破坏了 magnitude 阈值的区分力。

> [!IMPORTANT]
> **这组实验的论文意义：**
> 它证明了 BQ2 的 Sign-Magnitude 编码不是"简单粗暴的 2-bit"——
> 它暗合了现代对比学习嵌入的分布先验。
> 这也解释了为什么看起来更理论优雅的 RaBitQ（有严格的误差界证明）
> 在实际 LLM 嵌入上反而不如"土法" BQ2：
>
> **理论最优 ≠ 分布特异性最优。BQ2 的优势来自对数据分布的正确假设。**
...
> [!IMPORTANT]
> **最重要的论文定位：**
> 不是 “Binary Quantization for HNSW”。
...
> **日期**: 2026-05-02 ~ 2026-05-04
> **里程碑**: 从 Cohere-1M 单数据集扩展到九数据集全覆盖，完成 hnswlib 强基线对照，提炼四级适用性梯度，论文定稿并投稿 arXiv。

### 14.1 五维度 LLM Embedding 全覆盖

§13 仅在 Cohere-1M (768d) 和 DBpedia-1M (1536d) 上完成了验证。
Phase 5 补充了 MiniLM-1M (384d)、BGE-M3-1M (1024d)、DBpedia-3072 (3072d)，形成 **384 → 768 → 1024 → 1536 → 3072** 的完整维度梯度：

| 数据集       | 维度 | ef=64 Recall@10 | ef=64 MT-QPS | 建图时间 | 热内存   | 16T/1T比 |
| ------------ | ---- | --------------- | ------------ | -------- | -------- | -------- |
| MiniLM-1M    | 384  | 88.09%          | 41,106       | 83s      | 583 MB   | 9.5×     |
| Cohere-1M    | 768  | 95.13%          | 36,729       | 95s      | 675 MB   | 10.4×    |
| BGE-M3-1M    | 1024 | 93.81%          | 41,182       | 78s      | 736 MB   | 8.8×     |
| DBpedia-1M   | 1536 | 95.34%          | 22,035       | 142s     | 849 MB   | 6.8×     |
| DBpedia-3072 | 3072 | 95.65%          | 12,910       | 262s     | 1,212 MB | 5.4×     |

**关键发现——维度无关性再确认**：
维度从 384 翻 8 倍到 3072，MT-QPS 仅降 3.2×（远低于 float32 的 8× 线性衰减），
热内存仅增 2.1×。Recall 在所有对比学习 embedding 上均稳定在 88-96%。
多线程加速比随维度增加从 10.4× 降至 5.4×，反映了高维向量 rerank 的内存带宽瓶颈。

### 14.2 全方位 Baseline 对照（Cohere-1M, 768-d）

> **日期**: 2026-05-05 ~ 05-08  
> **硬件**: AMD Ryzen 7 7840HS, 16线程, 32GB DDR5-5600  
> **对照系统**: hnswlib (C++), FAISS HNSW (Python/C++), USearch (Rust), DiskANN (VSAG Rust), FAISS IVF-PQ (OPQ+Refine)  
> **方法**: 所有数据为同机公平测试，QuIVer 倍数通过**严格同 Recall 线性插值**计算。  
> **详细数据**: 见 [baseline_comparison.md](baseline_comparison.md)

#### 搜索性能：同 Recall 插值对比（MT-QPS, 16线程）

| Recall | QuIVer | DiskANN Rust | hnswlib | FAISS HNSW | USearch | OPQ+PQ+Refine |
|---|---|---|---|---|---|---|
| **~95%** | **36,729** | 11,557 (3.2×) | 8,500 (4.3×) | 7,600 (4.8×) | 6,700 (5.5×) | 5,399 (6.8×) |
| **~97%** | **21,021** | 8,047 (2.6×) | 5,900 (3.6×) | 5,500 (3.8×) | 4,900 (4.3×) | 2,715 (7.7×) |
| **~99%** | **11,837** | 3,538 (3.3×) | 2,500 (4.7×) | 2,400 (4.9×) | 2,200 (5.4×) | 1,501 (7.9×) |
| **~99.5%** | **6,032** | 2,433 (2.5×) | 1,350 (4.5×) | 1,280 (4.7×) | 1,150 (5.2×) | 441 (13.7×) |

#### 搜索性能：同 Recall 插值对比（1T-QPS, 单线程）

| Recall | QuIVer | DiskANN Rust | hnswlib | FAISS HNSW | USearch |
|---|---|---|---|---|---|
| **~95%** | **3,555** | 2,975 (1.2×) | 1,900 (1.9×) | 2,700 (1.3×) | 1,100 (3.2×) |
| **~97%** | **2,140** | 2,085 (1.0×) | 1,350 (1.6×) | 1,950 (1.1×) | 780 (2.7×) |
| **~99%** | **1,182** | 879 (1.3×) | 560 (2.1×) | 850 (1.4×) | 350 (3.4×) |
| **~99.5%** | **644** | 585 (1.1×) | 300 (2.1×) | 440 (1.5×) | 185 (3.5×) |

#### 构图对比

| 系统 | Build(s) | vecs/s | **QuIVer 加速** |
|---|---|---|---|
| **QuIVer** | **93s** | **10,764** | — |
| DiskANN Rust | 234~396s | 2,525~4,281 | **2.5~4.3×** |
...
|---|---|---|
| **QuIVer** | **675 MB** | — |
| hnswlib / FAISS / USearch | ~3,189 MB | **4.7×** |
| DiskANN Rust | ~3,200 MB | **4.7×** |
| FAISS IVF-PQ (裸) | ~104 MB | IVF-PQ 更省（但 recall 天花板 63%） |
| FAISS OPQ+Refine | ~3,000 MB | 加精排后内存与 HNSW 持平 |

> [!IMPORTANT]
> **关键结论：**
> 1. **DiskANN Rust 是最强外部图索引基线**：1T 下几乎追平 QuIVer（1.0-1.3×），但 MT 下仍落后 **2.5-3.3×**。
> 2. **hnswlib/FAISS/USearch 在 MT 下 QuIVer 快 3.6-5.5×**，1T 下快 1.1-3.5×。
> 3. **FAISS IVF-PQ（OPQ+Refine）加了 f32 精排后内存与 HNSW 持平（~3GB）**，失去唯一优势，且 QPS 全线垫底。
> 4. **QuIVer 热内存仅 675 MB**，比所有 HNSW 系基线省 **75%**。
> 5. **MT 倍数 >> 1T 倍数**的根源：QuIVer 热数据 675MB 可大部分驻留 L3 cache，16 线程下 cache 争用极低；而 HNSW 系 3.2GB 热数据远超 L3，并发下 DRAM 带宽成为瓶颈。

### 14.3 多模态 CLIP 实验：Wolt-CLIP-1M (512d)

**动机**：单模态 LLM text embedding 在 QuIVer 上 SOTA 级表现已被确认。
但 CLIP 模型将图像和文本向量映射到同一空间——这种跨模态混合是否会破坏 BQ 可导航性？

| ef  | Recall@10  | MT-QPS | 1T-QPS |
| --- | ---------- | ------ | ------ |
| 64  | **70.68%** | 62,367 | 7,152  |
| 128 | 77.40%     | 39,064 | 4,180  |
| 256 | 81.66%     | 22,560 | 2,510  |
| 512 | 84.28%     | 12,128 | 1,426  |

**关键发现**：

- 71% recall 远高于 Euclidean 坍缩（<6%），说明 CLIP 的 InfoNCE 对比训练仍然保留了可观的方向性结构
- 但低于单模态 LLM 的 88%+，因为图像/文本两种分布在共享空间中形成异质混合，增加了 BQ 排序噪声
- **这揭示了一个连续梯度**：不是「能用/不能用」的二元判断，而是一个从 SOTA → High → Usable → Collapse 的连续谱

### 14.4 因果分析：Synthetic-LR (低秩合成, 768d)

**动机**：§13.9 中 Random-Sphere (0.27%) 和 Cohere-1M (95%) 的巨大差异说明数据分布是关键。
但到底是**语义聚类**的作用，还是**低有效维度**的作用？需要一个控制变量实验。

**构造方法**：

- 768 维空间中，信号仅生活在 64 维正交子空间内
- 256 个 Zipf 分布聚簇，经随机正交基投影到 768 维
- 小噪声 ε=0.05 + L2 归一化
- **无语义内容**——纯粹的数学结构

| ef  | Recall@10  | MT-QPS | 1T-QPS |
| --- | ---------- | ------ | ------ |
| 64  | **41.76%** | 28,091 | 3,440  |
| 128 | 56.82%     | 15,562 | 1,826  |
| 256 | 70.33%     | 8,350  | 1,021  |
| 512 | 81.30%     | 4,442  | 548    |
...
```
                50%              +45%              = 95%
        ┌───────────────┐ ┌─────────────────┐ ┌──────────────┐
        │ 低秩流形结构   │ │ 对比学习几何     │ │ 真实LLM嵌入   │
        │ (Synthetic-LR) │ │ (InfoNCE独有)    │ │ (Cohere-1M)  │
        └───────────────┘ └─────────────────┘ └──────────────┘
```

> [!IMPORTANT]
> **这是整个研究最重要的因果证据之一。**
> Random-Sphere (0.3%) vs Synthetic-LR (50%)：唯一区别是低秩结构 → 低有效维度是**必要条件**。
> Synthetic-LR (50%) vs Cohere-1M (95%)：唯一区别是 InfoNCE 训练几何 → 对比学习几何是**充分条件的另一半**。
> 两者缺一不可。

### 14.5 四级适用性梯度

综合 11 个数据集的 ef=64 结果，形成清晰的四级分类：

| 级别         | 数据集                            | ef=64 R@10 | 特征                                 |
| ------------ | --------------------------------- | ---------- | ------------------------------------ |
| **SOTA**     | MiniLM, Cohere, BGE-M3, DBpedia×2 | 88~96%     | 单模态对比学习 LLM embedding         |
| **High**     | Wolt-CLIP                         | 71%        | 多模态对比学习，跨模态混合           |
| **Usable**   | GloVe, Synthetic-LR               | 32~42%     | 部分方向性结构（非对比训练或纯低秩） |
| **Collapse** | SIFT, GIST, Random-Sphere         | <15%       | Euclidean 原生或无结构               |

> [!NOTE]
> **这个梯度本身就是论文的核心 contribution 之一。**
> 它将 QuIVer 的适用边界从「能用/不能用」的模糊判断
> 精确化为一个基于数据分布的连续函数，并给出了因果机制。
...
从第一行 BQ 性能审计代码到最后一次 arXiv 提交，<br/>
5 天，2000+ 行，11 组数据集，5 次否定，5 个 Phase，1 篇论文。<br/><br/>
<b>「否定的价值不亚于发现。」</b><br/><br/>
2026-05-04 · TriviumDB Research Lab<br/>
<small>最后数据更新: 2026-05-08 · bench_sensitivity 全数据集最终版</small>
</i>
</p>
