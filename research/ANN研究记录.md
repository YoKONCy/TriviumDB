...
| MiniLM-1M (LLM, 384d)      | 1M      | ef=64           | 38942 QPS         | 71s         | ✅ SOTA         |
| DBpedia-1M (LLM, 1536d)    | 990K    | ef=64           | 16525 QPS         | 137s        | ✅ SOTA         |
| RedCaps-1M (CLIP, 512d)    | 1M      | 无法达到        | 36291 QPS (78%R)  | —           | 🔶 多模态高     |
| Synthetic-LR (低秩, 768d)  | 1M      | 无法达到        | 16269 QPS (50%R)  | —           | 🔶 因果验证     |
| Random-Sphere (随机, 768d) | 1M      | 无法达到        | 50186 QPS (0.3%R) | —           | ❌ 结构坍缩     |
...
| **hnswlib 对照 (768d)**    | **1M**  | **ef=64**       | **QuIVer 16×快**  | **351s**    | ✅ **基线对照** |
...
### 14.1 三维度 LLM Embedding 全覆盖

§13 仅在 Cohere-1M (768d) 和 DBpedia-1M (1536d) 上完成了验证。
Phase 5 补充了 MiniLM-1M (384d)，形成 **384 → 768 → 1536** 的完整维度梯度：

| 数据集     | 维度 | ef=64 Recall@10 | ef=64 QPS | 建图时间 | 热内存 |
| ---------- | ---- | --------------- | --------- | -------- | ------ |
| MiniLM-1M  | 384  | 91.20%          | 38,942    | 71s      | 583 MB |
| Cohere-1M  | 768  | 95.12%          | 25,068    | 103s     | 675 MB |
| DBpedia-1M | 1536 | 94.63%          | 16,525    | 137s     | 849 MB |

**关键发现——维度无关性再确认**：
维度从 384 翻 4 倍到 1536，QPS 仅降 2.4×（远低于 float32 的 4× 线性衰减），
热内存仅增 1.46×。BQ 距离代价在 AVX-512 下几乎不随维度增长。

### 14.2 hnswlib 强基线对照

这是**首次与 hnswlib 原版 C++ 实现**在相同硬件上直接对比。

| 系统       | ef  | 建图(s) | R@10       | QPS        | Speedup   |
| ---------- | --- | ------- | ---------- | ---------- | --------- |
| **QuIVer** | 64  | **103** | **95.12%** | **25,068** | —         |
| **QuIVer** | 128 | **103** | **97.69%** | **14,368** | —         |
| **QuIVer** | 256 | **103** | **98.95%** | **7,957**  | —         |
| hnswlib    | 32  | 351     | 90.01%     | 2,240      | 12.3×     |
| hnswlib    | 64  | 351     | 94.87%     | 1,607      | **17.2×** |
| hnswlib    | 128 | 351     | 97.60%     | 1,145      | 12.1×     |
| hnswlib    | 256 | 351     | 98.56%     | 610        | 13.0×     |
| USearch    | 32  | 687     | 85.37%     | 14,494     | 1.9×      |
| USearch    | 64  | 687     | 91.32%     | 10,354     | 2.7×      |
| USearch    | 128 | 687     | 94.27%     | 5,641      | 4.9×      |

> [!IMPORTANT]
> **在 ~95% Recall 点位：QuIVer 25.1K QPS vs hnswlib 1.6K QPS → 15.6× 加速。**
> 建图速度：QuIVer 103s vs hnswlib 351s → 3.4× 加速。
> 吞吐优势的根源：768d BQ 对称距离 <10ns vs 768d f32 点积 ~250ns → 每跳成本 20× 差距。

### 14.3 多模态 CLIP 实验：RedCaps-1M (512d)
...
- 78% recall 远高于 Euclidean 坍缩（<6%），说明 CLIP 的 InfoNCE 对比训练仍然保留了可观的方向性结构
- 但低于单模态 LLM 的 91%+，因为图像/文本两种分布在共享空间中形成异质混合，增加了 BQ 排序噪声
...
综合九个数据集的 ef=64 结果，形成清晰的四级分类：

| 级别         | 数据集                  | ef=64 R@10 | 特征                                 |
| ------------ | ----------------------- | ---------- | ------------------------------------ |
| **SOTA**     | MiniLM, Cohere, DBpedia | 91~95%     | 单模态对比学习 LLM embedding         |
| **High**     | RedCaps (CLIP)          | 78%        | 多模态对比学习，跨模态混合           |
| **Usable**   | GloVe, Synthetic-LR     | 50~55%     | 部分方向性结构（非对比训练或纯低秩） |
| **Collapse** | SIFT, GIST, Random      | <6%        | Euclidean 原生或无结构               |
...
5 天，2000+ 行，9 组数据集，5 次否定，5 个 Phase，1 篇论文。<br/><br/>
<b>「否定的价值不亚于发现。」</b><br/><br/>
2026-05-04 · TriviumDB Research Lab
</i>
</p>
